...
def append_counter_section(report_lines: list[str], title: str, counter: MetricCounter, limit: int | None = None) -> None:
...
def build_report(scope_name: str, lists_for_scope: list[ListData], metrics: ScopeMetrics) -> str:
    assumptions = [
    ...
    append_counter_section(report_lines, "## Top units by entries", metrics.unit_entries, REPORT_SECTION_LIMIT)
    report_lines.append("## Top units by list presence")
    for name, list_count in metrics.unit_presence_lists.most_common(REPORT_SECTION_LIMIT):
    ...
    report_lines.append("")
    append_counter_section(report_lines, "## Top units by model count", metrics.model_counts, REPORT_SECTION_LIMIT)

    report_lines.append("## Unplayed units")
    if metrics.unplayed_units:
        report_lines.extend([f"- {name} (unit size {size})" for name, size in metrics.unplayed_units])
    else:
        report_lines.append("- None")
    report_lines.append("")

    append_counter_section(report_lines, "## Subfaction counts", metrics.subfactions)
    append_counter_section(report_lines, "## Manifestation lore counts", metrics.manifestation_lores)
    append_counter_section(report_lines, "## Artifact counts", metrics.artifacts)
    append_counter_section(report_lines, "## Command trait counts", metrics.command_traits)
    append_counter_section(report_lines, "## Warmachine trait counts", metrics.warmachine_traits)
